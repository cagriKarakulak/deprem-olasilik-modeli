...
# Türkiye ve çevresi için yaklaşık sınırlayıcı kutu
MIN_LATITUDE = 35.0
MAX_LATITUDE = 43.0
MIN_LONGITUDE = 25.0
MAX_LONGITUDE = 45.0

# Grid Hücresi Boyutu (derece cinsinden)
GRID_RES_LAT = 0.2 # Yaklaşık 22 km
GRID_RES_LON = 0.2 # Yaklaşık 16-18 km (enleme göre değişir)

# Veri Çekme Zaman Aralığı
DATA_YEARS = 50 # 50 yillik data
END_TIME = datetime.now(timezone.utc)
START_TIME = END_TIME - timedelta(days=DATA_YEARS*365.25) #.25 artik gunler icin

USGS_API_BASE_URL = "https://earthquake.usgs.gov/fdsnws/event/1/query"

# Özellik Mühendisliği ve Tahmin Pencereleri
FEATURE_WINDOW_DAYS = 90
PREDICTION_WINDOW_DAYS = 30
TIME_STEP_DAYS = 7

# Tahmin Edilecek Büyük Deprem Eşiği
MAGNITUDE_THRESHOLD = 5.0

# b-değeri Hesaplama Ayarları
B_VALUE_MIN_QUAKES = 25
B_VALUE_MC_METHOD = 'MAXC'

# Gelecek Tahmini İçin Olasılık Eşiği
FUTURE_PREDICTION_PROB_THRESHOLD = 0.6
PREDICTION_OUTPUT_FILE = f"deprem_tahminleri_{END_TIME.strftime('%Y%m%d_%H%M')}.txt"
...
# --- Özellik İsimleri ---
feature_names = [
    "Deprem Sayısı", "Ort. Büyüklük", "Maks. Büyüklük", "Enerji Vekili",
    "Ort. Zaman Farkı (sn)", "Std. Zaman Farkı (sn)", "b-değeri", "Mc (Tamamlama Mag.)", "Ort. Derinlik"
]
...
def fetch_usgs_data_paginated(start_time, end_time, min_lat, max_lat, min_lon, max_lon, min_mag=1.0):
    """ Belirtilen zaman/bölge için USGS'den sayfalı veri çeker. """
    all_features_list = []
    offset = 1
    limit = 20000
    total_fetched = 0
    ...
    while True:
        ...
        while retries < max_retries:
            try:
                ...
                print(f"\nHata: HTTP (offset {offset}): {e.response.status_code} {e.response.reason}")
                response_text = ""
        ...
        if not request_successful and retries < max_retries: print("Kurtarılamayan hata. Durduruldu."); break
    ...
    for feature in all_features_list:
        ...
        try:
            ...
            dt_object = datetime.fromtimestamp(time_ms / 1000.0, tz=timezone.utc) if time_ms is not None else None
            mag = prop.get('mag')
...
def generate_features_for_cells(target_time, earthquake_df, lat_bins, lon_bins, feature_window_days, b_value_min_quakes):
    """ Belirli bir zaman için tüm hücrelerin özelliklerini hesaplar. """
    feature_start = target_time - timedelta(days=feature_window_days)
    feature_end = target_time
    relevant_quakes = earthquake_df[
        (earthquake_df['timestamp'] >= feature_start) &
        (earthquake_df['timestamp'] < feature_end)
    ].copy()
    ...
    if relevant_quakes.empty:
        cell_features[['Deprem Sayısı', 'Enerji Vekili', 'Ort. Büyüklük', 'Maks. Büyüklük']] = 0
    ...
    mean_mag = grouped['magnitude'].mean(); max_mag = grouped['magnitude'].max(); mean_depth = grouped['depth'].mean()
    ...
    if not mean_depth.empty: cell_features.loc[mean_depth.index, 'Ort. Derinlik'] = mean_depth
    ...
    if not time_stats.empty:
        cell_features.loc[time_stats.index, 'Ort. Zaman Farkı (sn)'] = time_stats.apply(lambda x: x[0])
        cell_features.loc[time_stats.index, 'Std. Zaman Farkı (sn)'] = time_stats.apply(lambda x: x[1])
    ...
    if not b_mc_values.empty:
        cell_features.loc[b_mc_values.index, 'b-değeri'] = b_mc_values.apply(lambda x: x[0])
        cell_features.loc[b_mc_values.index, 'Mc (Tamamlama Mag.)'] = b_mc_values.apply(lambda x: x[1])

    cell_features['Deprem Sayısı'] = cell_features['Deprem Sayısı'].fillna(0)
    cell_features['Enerji Vekili'] = cell_features['Enerji Vekili'].fillna(0)
    cell_features['Ort. Büyüklük'] = cell_features['Ort. Büyüklük'].fillna(0)
    cell_features['Maks. Büyüklük'] = cell_features['Maks. Büyüklük'].fillna(0)

    for col in cell_features.columns:
        if cell_features[col].dtype != np.float32:
             cell_features[col] = pd.to_numeric(cell_features[col], errors='coerce').astype(np.float32)
    return cell_features
...
earthquake_df_raw = fetch_usgs_data_paginated(START_TIME, END_TIME, MIN_LATITUDE, MAX_LATITUDE, MIN_LONGITUDE, MAX_LONGITUDE, min_mag=1.5)

if earthquake_df_raw is None or earthquake_df_raw.empty: exit("Veri çekilemedi veya boş. Program sonlandırılıyor.")
earthquake_df_raw['timestamp'] = pd.to_datetime(earthquake_df_raw['timestamp'])
...
loop_start_time = earthquake_df_raw['timestamp'].min() + timedelta(days=FEATURE_WINDOW_DAYS)
if loop_start_time > earthquake_df_raw['timestamp'].max(): exit("Hata: Özellik penceresi veri aralığından daha uzun.")
loop_end_time = earthquake_df_raw['timestamp'].max() - timedelta(days=PREDICTION_WINDOW_DAYS)
if loop_start_time > loop_end_time:
    print(f"Uyarı: Veri aralığı ({DATA_YEARS} yıl) özellik ({FEATURE_WINDOW_DAYS} gün) ve tahmin ({PREDICTION_WINDOW_DAYS} gün) pencereleri için yeterince uzun değil.");
    loop_end_time = loop_start_time - timedelta(days=1)
...
while current_time <= loop_end_time:
    step_count += 1
    features_df = generate_features_for_cells(current_time, earthquake_df_raw, lat_bins, lon_bins, FEATURE_WINDOW_DAYS, B_VALUE_MIN_QUAKES)

    prediction_start = current_time
    prediction_end = current_time + timedelta(days=PREDICTION_WINDOW_DAYS)
    prediction_quakes = earthquake_df_raw[(earthquake_df_raw['timestamp'] > prediction_start) & (earthquake_df_raw['timestamp'] <= prediction_end)].copy()
    ...
    step_data = features_df.copy(); step_data['target'] = targets
    step_data_reset = step_data.reset_index()
    step_data_clean = step_data_reset.copy()
    for col in feature_names:
        if step_data_clean[col].isnull().any():
            median_val = step_data_clean[col].median()
            if pd.isnull(median_val): median_val = 0
            step_data_clean[col] = step_data_clean[col].fillna(median_val)

    if not step_data_clean.empty:
        all_step_features.append(step_data_clean[feature_names].values.astype(np.float32))
        all_step_targets.append(step_data_clean['target'].values.astype(np.int8))
        all_step_timestamps.extend([current_time] * len(step_data_clean))
        all_step_indices.append(step_data_clean[['lat_bin', 'lon_bin']].values.astype(np.int16))

    feature_pbar.update(1)
    if step_count % 50 == 0: feature_pbar.set_description(f"Özellik Hesabı {current_time.date()}")
    del features_df, targets, prediction_quakes, step_data, step_data_reset, step_data_clean
...
feature_pbar.close()
end_loop_time_proc = time.time()
print(f"...Özellik mühendisliği tamamlandı ({end_loop_time_proc - start_loop_time:.1f} saniye).")
...
try:
    ...
    timestamps = np.array(all_step_timestamps); cell_indices = np.concatenate(all_step_indices, axis=0).astype(np.int16)
except ValueError as e: exit(f"Hata: Numpy dizileri birleştirilemedi: {e}")
del all_step_features, all_step_targets, all_step_timestamps, all_step_indices; gc.collect()
...
print(f"Hedef 'Büyük Deprem Yok (0)': {target_counts[0] if len(target_counts)>0 else 0}")
print(f"Hedef 'Büyük Deprem Var (1)': {target_counts[1] if len(target_counts)>1 else 0}")
if len(target_counts) < 2 or target_counts[1] == 0: exit("\nHedef sınıfta (1) hiç örnek bulunamadı! Model eğitilemiyor.")

# --- 6. Zaman Serisi Çapraz Doğrulama ile Model Eğitimi ---
print("\nZaman Serisi Çapraz Doğrulama ile XGBoost Modeli Eğitimi...")
n_splits = 5; tscv = TimeSeriesSplit(n_splits=n_splits); fold_metrics = []
pos_count = (y == 1).sum(); neg_count = (y == 0).sum()
...
num_boost_round = 150; early_stopping_rounds = 15
all_test_preds, all_test_true = [], []; final_model = None; best_iteration_from_cv = num_boost_round
...
for fold, (train_index, test_index) in enumerate(cv_pbar):
    cv_pbar.set_description(f"CV Katman {fold + 1}/{n_splits}")
    X_train, X_test = X[train_index], X[test_index]
    y_train, y_test = y[train_index], y[test_index]
    if len(X_test) == 0: print(" Test seti boş, atlanıyor."); continue
    ...
    fold_metrics.append({'acc': accuracy, 'prec': precision, 'rec': recall, 'f1': f1, 'auc': roc_auc})
    all_test_preds.extend(y_pred_proba); all_test_true.extend(y_test)
...
# --- 8. Final Model Eğitimi (Tüm Veriyle) ---
print("\n--- Final Model Eğitimi (Tüm Veriyle) ---")
if len(X) > 0:
    print(f"Final eğitim seti boyutu: {len(X)}")
    dfinaltrain = xgb.DMatrix(X, label=y, feature_names=feature_names)
    watchlist_final = [(dfinaltrain, 'train')]
    final_num_rounds = max(50, best_iteration_from_cv + 10)
    print(f"Final model {final_num_rounds} tur ile eğitiliyor...")
    ...
    try:
        ...
        if importance:
             ...
             mapped_importance = {fmap.get(f, f): score for f, score in importance.items()}
             sorted_importance = sorted(mapped_importance.items(), key=lambda item: item[1], reverse=True)
             for feature, score in sorted_importance: print(f"- {feature}: {score:.2f}")
        else: print("Önem skorları alınamadı.")
    except Exception as e: print(f"Özellik önemi alınırken hata: {e}")
    del X, y, timestamps, cell_indices, dfinaltrain; gc.collect()
else: print("Final modeli eğitmek için yeterli veri yok."); final_model = None

# --- 9. GELECEK TAHMİNİ (SONRAKİ 1 AY) ---
print("\n--- GELECEK TAHMİNİ (SONRAKİ 30 GÜN) ---")
print(f"Uyarı: Bu tahminler DENEYSELDİR ve modelin geçmiş verilere dayanarak hesapladığı *olasılıklardır*.")
print(f"Gerçek depremleri veya kesin büyüklükleri göstermezler. Sadece M{MAGNITUDE_THRESHOLD}+ deprem olasılığıdır.")

high_prob_cells_output = []

# !!! YENİ: Geocoder başlatma (geopy varsa) !!!
geolocator = None
reverse = None
if GEOPY_AVAILABLE:
    try:
        # Nominatim'i başlat (OpenStreetMap verisi kullanır)
        geolocator = Nominatim(user_agent="my_earthquake_predictor_v1")
        # RateLimiter API'yi aşırı kullanmamak için önemlidir (Nominatim için genellikle 1 saniye bekleme)
        reverse = RateLimiter(geolocator.reverse, min_delay_seconds=1.1, return_value_on_exception=None)
        print("Geocoder (Nominatim) başarıyla başlatıldı.")
    except Exception as e:
        print(f"Hata: Geocoder başlatılamadı: {e}")
        GEOPY_AVAILABLE = False

if final_model is not None:
    print(f"\n'{END_TIME.date()}' tarihi itibarıyla gelecek {PREDICTION_WINDOW_DAYS} gün için özellikler hesaplanıyor...")
    now_features_df = generate_features_for_cells(END_TIME, earthquake_df_raw, lat_bins, lon_bins, FEATURE_WINDOW_DAYS, B_VALUE_MIN_QUAKES)
    now_features_filled_df = now_features_df.copy()
    for col in feature_names:
         if now_features_filled_df[col].isnull().any():
             now_features_filled_df[col] = now_features_filled_df[col].fillna(0)

    if not now_features_filled_df.empty:
        print(f"{len(now_features_filled_df)} hücre için özellikler hazırlandı.")
        X_now = now_features_filled_df[feature_names].values.astype(np.float32)
        dnow = xgb.DMatrix(X_now, feature_names=feature_names)
        print("Olasılıklar tahmin ediliyor...")
        try:
             best_iter = getattr(final_model, 'best_iteration', final_num_rounds)
             future_probabilities = final_model.predict(dnow, iteration_range=(0, best_iter))
        except Exception as e: print(f"Gelecek tahmini sırasında hata: {e}"); future_probabilities = None

        if future_probabilities is not None:
            predictions_df = now_features_filled_df.reset_index()
            predictions_df['probability'] = future_probabilities
            high_prob_cells = predictions_df[predictions_df['probability'] >= FUTURE_PREDICTION_PROB_THRESHOLD].sort_values('probability', ascending=False)

            if not high_prob_cells.empty:
                print(f"Belirtilen eşiği ({FUTURE_PREDICTION_PROB_THRESHOLD*100:.0f}%) geçen {len(high_prob_cells)} hücre bulundu.")
                print("Bölge isimleri alınıyor (internet bağlantısı gerektirir)...")

                geo_pbar = tqdm(high_prob_cells.iterrows(), total=len(high_prob_cells), desc="Bölge İsimleri")
                for _, row in geo_pbar:
                    lat_idx = int(row['lat_bin'])
                    lon_idx = int(row['lon_bin'])
                    prob = row['probability']
                    region_name = "Bilinmiyor" # Varsayılan
                    gmaps_link = "N/A"

                    if 0 <= lat_idx < len(lat_centers) and 0 <= lon_idx < len(lon_centers):
                        lat = lat_centers[lat_idx]
                        lon = lon_centers[lon_idx]
                        gmaps_link = f"https://www.google.com/maps?q={lat:.4f},{lon:.4f}"

                        if GEOPY_AVAILABLE and reverse is not None:
                            try:
                                # Koordinatı string olarak ver
                                location = reverse(f"{lat:.6f}, {lon:.6f}", language='tr', addressdetails=True, timeout=10)
                                if location:
                                    # Adres detaylarından il, ilçe, köy gibi bilgileri almaya çalış
                                    address = location.raw.get('address', {})
                                    parts = [address.get(k) for k in ['village', 'town', 'suburb', 'county', 'state', 'country'] if address.get(k)]
                                    region_name = ", ".join(parts) if parts else location.address
                                else:
                                    region_name = "Bölge bulunamadı"
                            except Exception as geo_e:
                                print(f"\nUyarı: Geocoding hatası ({lat:.2f},{lon:.2f}): {geo_e}")
                                region_name = "Geocoder Hatası"
                        else:
                            region_name = "Geocoder Yok"

                        high_prob_cells_output.append({
                            'latitude': lat,
                            'longitude': lon,
                        ...
                        print(f"Uyarı: Geçersiz hücre indeksi atlandı: ({lat_idx}, {lon_idx})")
                geo_pbar.close()
                print("Bölge isimleri alma işlemi tamamlandı.")

            else:
                print(f"Belirtilen eşiği ({FUTURE_PREDICTION_PROB_THRESHOLD*100:.0f}%) geçen yüksek olasılıklı hücre bulunamadı.")
        else: print("Gelecek olasılıkları hesaplanamadı.")
    else: print("Gelecek tahmini için geçerli özelliklere sahip hücre bulunamadı.")
else: print("Final model eğitilemediği için gelecek tahmini yapılamıyor.")
...
if high_prob_cells_output:
    ...
    try:
        with open(PREDICTION_OUTPUT_FILE, 'w', encoding='utf-8') as f:
            ...
            f.write(f"# Tahmin Tarihi: {END_TIME.strftime('%Y-%m-%d %H:%M:%S UTC')}\n")
            f.write(f"# Olasılık Eşiği: {FUTURE_PREDICTION_PROB_THRESHOLD*100:.0f}%\n")
            f.write("# UYARI: Bu tahminler deneyseldir ve bilimsel kesinlik taşımaz.\n")
            f.write("-" * 120 + "\n")
            f.write(f"{'Enlem':<10} {'Boylam':<10} {'Olasılık (%)':<15} {'Tahmini Bölge':<60} {'Google Haritalar Linki'}\n")
            f.write("-" * 120 + "\n")

            for prediction in high_prob_cells_output:
                lat_str = f"{prediction['latitude']:.2f}"
                lon_str = f"{prediction['longitude']:.2f}"
                prob_str = f"{prediction['probability']:.1f}"
                region_str = prediction['region'][:58]
                link_str = prediction['gmaps_link']
                f.write(f"{lat_str:<10} {lon_str:<10} {prob_str:<15} {region_str:<60} {link_str}\n")

        print("Tahminler başarıyla dosyaya yazıldı.")
    except IOError as e:
        print(f"Hata: Tahminler dosyaya yazılamadı: {e}")
    except Exception as e:
        print(f"Hata: Dosyaya yazma sırasında beklenmedik bir hata oluştu: {e}")
else:
    print("\nDosyaya yazılacak yüksek olasılıklı tahmin bulunmuyor.")


print("\n--- PROGRAM SONU ---")
print("!!! TEKRAR UYARI: Bu sonuçlar bilimsel kesinlik taşımaz ve gerçek deprem tahmini değildir !!!")
print("!!! Modelin performansı sınırlıdır ve hatalar içerebilir (gözden kaçırma ve yanlış alarmlar) !!!")